...
impl CoworkAgent {
    ...
    pub async fn parse_instruction(
        ...
        let ai_response = {
            ...
            provider
                .execute_prompt(
                    &ProviderType::Gemini, // Use available provider
                    "gemini-2.5-flash",
                    &prompt,
                    |_, _| {},
                )
        ...
        let plan = self.parse_ai_response(&ai_response, instruction)?;
    ...
    fn build_planning_prompt(&self, instruction: &str, context: &WorkspaceContext) -> String {
        format!(
            r#"You are an AI file management assistant. Parse the user's instruction and generate a structured plan.
...
RULES:
1. Only suggest operations that match the user's intent
2. Set requires_confirmation=true for any delete or modify operations
3. Include clear descriptions for each step
4. If the instruction is ambiguous, provide the safest interpretation
5. For "organize", prefer "by_type" strategy unless user specifies otherwise
