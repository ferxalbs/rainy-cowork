...
impl KeychainManager {
    ...
    pub fn store_key(&self, provider: &str, api_key: &str) -> Result<(), String> {
        ...
        {
            // TODO: Implement cross-platform secure storage (e.g. using keyring crate)
            // For now, we return an error on non-macOS
            let _ = provider;
            let _ = api_key;
            Err("Secure storage is currently only available on macOS".to_string())
        }
    }
